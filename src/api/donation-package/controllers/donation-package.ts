...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async createPaypalPayment(ctx) {
      try {
        const body = (ctx.request.body ?? {}) as CreatePaypalPaymentBody;
        ...
        if (!Number.isFinite(totalOrder) || totalOrder <= 0) {
          return ctx.badRequest('`total_order` must be a number > 0.');
        }

        if (!Number.isFinite(totalPrice) || totalPrice <= 0) {
          return ctx.badRequest('`total_price` must be a number > 0.');
        }
        ...
        strapi.log.error('createPaypalPayment controller failed.', error);
        return ctx.internalServerError('Failed to create PayPal payment link.');
      }
    },
    ...
    async createBankTransferDonation(ctx) {
      try {
        const body = (ctx.request.body ?? {}) as CreateBankTransferDonationBody;
        const itemsFromBody = Array.isArray(body.items) ? body.items : [];
