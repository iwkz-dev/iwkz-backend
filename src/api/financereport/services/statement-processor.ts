...
const fetchLedgerRows = async (strapi: any) => {
  ...
  while (hasNextPage) {
    try {
      ...
      strapi.log.error('Failed to fetch finance ledgers from NocoDB.', error);
      throw createApplicationError(
  ...
  for (const ledgerConfig of ledgerConfigs) {
    for (const subCategory of ledgerConfig.subCategories) {
      for (const keyword of subCategory.keywords) {
        if (
          keyword &&
          (createWordBoundaryRegex(keyword).test(normalizedDescription) ||
...
export const createStatementProcessor = ({ strapi }: { strapi: any }) => ({
  async processStatement(payload: any) {
    ...
    const transactions = payload.data.map((transaction: any, index: number) => {
      const amount = Math.abs(Number(transaction.amount) || 0);
      const isIncome = transaction.status === 'income';
      const { month, monthNumber, year } = getDateParts(transaction.date, '-');
      const monthLabel = getMonthLabel(monthNumber);
      const cleanedDescription = cleanUpDescription(transaction.description);
    ...
    const transactionsSaveToDb = transactions.map((transaction: any) => {
      const outcome =
        transaction.ausgabe === 'x' ? toCurrencyNumber(transaction.soll) : 0;

      const income =
        transaction.einnahme === 'x' ? toCurrencyNumber(transaction.haben) : 0;
      return {
    ...
    const normalizedTransactionCashflow = {
      year: Number(
        transactionCashflow.year ?? getDateParts(payload.date, '.').year
      ),
